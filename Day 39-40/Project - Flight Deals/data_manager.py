import os
import requests
from dotenv import load_dotenv
from flight_data import FlightData
from notification_manager import NotificationManager
import json
load_dotenv()
EXCEL_URL = os.getenv("EXCEL_URL")
EXCEL_KEY = os.getenv("EXCEL_KEY")


class DataManager(FlightData, NotificationManager):
    def __init__(self):
        super().__init__()

        self.data = self.get_exel_data()


    # Hämtar data från Google Sheet, fallback till defaultdata, sparar till self.data
    def get_data_google_sheet(self):
        try:
            response = requests.get(EXCEL_URL)
            response.raise_for_status()
            get_data = response.json()
            data = get_data["prices"]

        except requests.RequestException:
            data = [{
            "city": "Paris",
            "iataCode": "PAR",
            "lowestPrice": 1000,
            "id": 1
        }]

        with open("sheet_data.txt", mode="w") as file:
            json.dump(data, file, indent=4)

        return data


    # Läser lokal excel-fil, skapar fil om den saknas
    def get_exel_data(self):
        try:
            with open("sheet_data.txt", mode="r") as file:
               data =  json.load(file)

        except FileNotFoundError:
              data =  self.get_data_google_sheet()

        return data



    # Uppdaterar pris för en rad i Google Sheet och synkroniserar lokalt
    def update_exel(self, row_id, new_price):
        try:
            body = {
                "price": {
                    "lowestPrice": new_price,
                }
            }

            response = requests.put(f"{EXCEL_URL}/{row_id}", json=body)
            response.raise_for_status()
        except requests.RequestException:
            print(f"Failed to update Google Sheet")

        # Uppdaterar alltid den lokala filen
        self.update_local_exel(row_id, new_price)



    # Uppdaterar pris för en rad i den lokala filen
    def update_local_exel(self, row_id, new_price):

        data = self.get_exel_data()

        for row in data:
            if row["id"] == row_id:
                row["lowestPrice"] = new_price


        with open("sheet_data.txt", "w", encoding="utf-8") as f:
            json.dump(data, f, indent=4)

        self.data = data
        print(f"Local file updated: id: {row_id}, new price: {new_price}")







